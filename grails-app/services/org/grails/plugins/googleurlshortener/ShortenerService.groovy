...
class ShortenerService {
    static transactional = true //ORLY?
    static final restUri = 'https://www.googleapis.com/urlshortener/v1/'
    def grailsApplication

    URLResource shorten(String url) {
        def requestMap = [:]
        if (grailsApplication.config.google?.shortener?.api?.key) {
            requestMap.put 'key', grailsApplication.config.google.shortener.api.key
        }
    ...
    def parseDetailsFragment(JSONArray jsonArray) {
